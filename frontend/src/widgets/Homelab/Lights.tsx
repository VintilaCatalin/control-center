  ...
  if (!data?.configured) {
    return (
      <div className={styles.state}>
        <span className={styles.stateBody}>Home Assistant isn't connected. Add a URL and token in Settings to control your lights here.</span>
      </div>
    );
  }
  if (data.error) {
    return (
      <div className={styles.state}>
        <span className={styles.stateBody}>{data.error}</span>
      </div>
    );
  }
