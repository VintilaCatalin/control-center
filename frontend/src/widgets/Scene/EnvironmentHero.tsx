...
export function EnvironmentHero() {
  ...
  // Every action below (palette strip, swatches, mode buttons, sliders)
  // ends up calling the same Home Assistant-backed routes - with nothing
  // connected there's nothing for them to do but fail, so a fresh install
  // gets one clear "connect this" message instead of a full control panel
  // wired to nothing. The wallpaper/palette itself still shows regardless
  // - Scene's wallpaper features don't need lighting configured at all.
  const lightsConfigured = snapshot?.lights?.configured ?? false;
    ...
    <div className={styles.hero}>
      ...
      <div className={styles.content}>
        <div className={styles.systemRow}>
          {palette.length > 0 &&
            (lightsConfigured ? (
              ...
              <div className={styles.paletteStrip} style={{ background: `linear-gradient(to right, ${palette.join(', ')})` }} />
            ))}
          {lightsConfigured && palette.length > 0 && (
        ...
        {lightsConfigured ? (
          ...
          <div className={styles.controlsRow}>
            <span className={styles.lightStatusEmpty}>Connect Home Assistant in Settings to control lighting from here.</span>
          </div>
        )}
      </div>
    </div>
