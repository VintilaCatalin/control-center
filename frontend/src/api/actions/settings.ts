...
// POST /api/settings/onboarding-complete - flips the one persisted flag
// that decides whether App.tsx shows the onboarding flow or the real
// shell. "Skip everything" and "connected three things" both call this
// the same way - onboarding doesn't track which integrations got set up,
// that's already visible from each one's own settings values.
export function completeOnboarding(): Promise<{ ok: boolean }> {
  return postAction('/api/settings/onboarding-complete', {});
}
