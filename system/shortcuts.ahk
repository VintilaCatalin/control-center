...
vdaPath    := A_ScriptDir "\..\VirtualDesktopAccessor.dll"  ; one level up: this script lives in system/, the DLL stays at the repo root
...
; These three used to open their own standalone Python windows
; (wallpicker.py/wallhaven.py/lights.py's own Tkinter GUIs). Control
; Center's Scene view now covers all three (YoursLibrary, WallhavenLibrary,
; EnvironmentHero's lighting controls) with real backend-wired equivalents,
; so the shortcut opens/focuses Control Center on Scene instead of a
; second, duplicate window. control_center.py itself handles "already
; open -> focus it" vs. "not running -> launch it" (see focus_window() /
; find_window() there) - the shortcut doesn't need to know which case it
; is. The underlying scripts stay exactly where they are; Control Center's
; backend still subprocess-invokes them for the headless actions (applying
; a wallpaper, --colorful, --fix-desktops, etc.) that never needed a
; window in the first place.
^+w:: Run('pythonw.exe "' A_ScriptDir '\..\control_center.py" --view scene',, "Hide")
^+a:: Run('pythonw.exe "' A_ScriptDir '\..\control_center.py" --view scene',, "Hide")
^+l:: Run('pythonw.exe "' A_ScriptDir '\..\control_center.py" --view scene',, "Hide")
    ...
    if (isFull) {
        ...
        if (isBrowser) {
            if (HueSyncState != "Video") {
                ...
                Run('pythonw.exe "' A_ScriptDir '\lights.py" --video',, "Hide")
                Sleep 500
            ...
            if (HueSyncState != "Game") {
                ...
                Run('pythonw.exe "' A_ScriptDir '\lights.py" --game',, "Hide")
                Sleep 500
        ...
        if (HueSyncState != "None") {
            ...
            Run('pythonw.exe "' A_ScriptDir '\lights.py" --auto',, "Hide")
        }
    }
}
